...
import { mkdir, cp, readdir, stat, readFile, writeFile } from "node:fs/promises";
...
const TARGETS = {
  ...
  "gemini":       { path: ".gemini/skills",  label: "Gemini CLI / Code Assist", emitContextFile: "GEMINI.md" },
};
...
async function installToTarget(targetKey, scope) {
  ...
  // For agents that don't natively load SKILL.md (e.g. Gemini), also emit a
  // top-level context file (GEMINI.md) so the model actually picks up the skill.
  if (target.emitContextFile) {
    const skillBody = await readFile(join(SKILL_SRC, "SKILL.md"), "utf8");
    const contextPath = join(base, target.emitContextFile);
    const header = `<!-- Auto-generated by mor-bio-examiner CLI. Edits will be overwritten on reinstall. -->\n` +
      `<!-- Source: node_modules/mor-bio-examiner/skill/SKILL.md -->\n\n`;
    await writeFile(contextPath, header + skillBody, "utf8");
  }
